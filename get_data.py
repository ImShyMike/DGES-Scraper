...
def build_candidate(stats: Dict[str, Any]) -> Optional[CandidateStats]:
...
for n, course in enumerate(courses):
    ...
    if table:
        ...
        parsed_years: Dict[str, YearData] = {}
        for year, phases in data_struct.items():
            phase1 = build_phase(phases.get("1ª Fase", {}))
            phase2 = build_phase(phases.get("2ª Fase", {}))
            parsed_years[year] = YearData(
                phase1=phase1, phase2=phase2 if has_phase_headers else None
            )
    ...
    database.append(
        CourseData(
            course=from_json(stringified_course),
            previous_data=parsed_years,
            characteristics=characteristics,
